...
#boston_shuffle = np.random.permutation(boston_shuffle) # shuffel along first axis only
#
############ Spliting X and Y again ##################
#boston_X_data = boston_shuffle[:,:-1]
#boston_Y_data = boston_shuffle[:,-1]

#percent_data = np.array([70,80,90,99],np.int)*len(boston_X_data)/100
...
#    np.random.shuffle(boston_shuffle) # shuffel along first axis only
    ...
    ########## Least square regresion fitting #############
#    boston_least_reg = linear_model.LinearRegression(fit_intercept=True, normalize=False)
#    boston_least_reg.fit(boston_X_train, boston_Y_train)
...
plt.suptitle('Ridge regression with own code')
fig_count=240
for m in percent_data:
