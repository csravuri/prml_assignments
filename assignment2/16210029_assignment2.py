...
# Total time 50 Sec approx
# Refered this link to understand Credit card data
...
def topNaccuracy(proba, Y_true, N):
    ...
    for i in range(proba.shape[0]):
        ...
        b = b[:N]
        # discarding classes with zero prabablities Ex: 0.8, 0.2, 0, 0, 0, ...
        if (b.count(0)):
            b = b[:b.index(0)]
...
credit_edu = (credit_card.EDUCATION == 6) | (credit_card.EDUCATION == 0)
credit_card.loc[credit_edu, 'EDUCATION'] = 5

credit_PAY_0 = (credit_card.PAY_0 == -2) | (credit_card.PAY_0 == -1)
credit_card.loc[credit_PAY_0, 'PAY_0'] = 0

credit_PAY_2 = (credit_card.PAY_2 == -2) | (credit_card.PAY_2 == -1)
credit_card.loc[credit_PAY_2, 'PAY_2'] = 0

credit_PAY_3 = (credit_card.PAY_3 == -2) | (credit_card.PAY_3 == -1)
credit_card.loc[credit_PAY_3, 'PAY_3'] = 0

credit_PAY_4 = (credit_card.PAY_4 == -2) | (credit_card.PAY_4 == -1)
credit_card.loc[credit_PAY_4, 'PAY_4'] = 0

credit_PAY_5 = (credit_card.PAY_5 == -2) | (credit_card.PAY_5 == -1)
credit_card.loc[credit_PAY_5, 'PAY_5'] = 0

credit_PAY_6 = (credit_card.PAY_6 == -2) | (credit_card.PAY_6 == -1)
credit_card.loc[credit_PAY_6, 'PAY_6'] = 0

credit = credit_card.values

credit_X1 = credit[:,1:-1] # discarding col 'ID'
credit_Y = credit[:,-1]

# removed categerial features from credit_X1 data 
credit_X = np.delete(credit_X1,[1,2,3,5,6,7,8,9,10], axis=1)
credit_noncat_len = credit_X.shape[1]

credit_prepro = OneHotEncoder(categorical_features='all')

credit_prepro.fit(np.array([1,2]).reshape(-1,1)) # SEX
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,1].reshape(-1,1)).toarray()), axis=1)

credit_prepro.fit(np.array([1,2,3,4,5]).reshape(-1,1)) # EDUCATION
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,2].reshape(-1,1)).toarray()), axis=1)


credit_prepro.fit(np.array([1,2,3]).reshape(-1,1)) # MARRIAGE
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,3].reshape(-1,1)).toarray()), axis=1)


credit_prepro.fit(np.arange(9).reshape(-1,1)) # PAY 0, 2 , 3, 4
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,5].reshape(-1,1)).toarray()), axis=1)
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,6].reshape(-1,1)).toarray()), axis=1)
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,7].reshape(-1,1)).toarray()), axis=1)
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,8].reshape(-1,1)).toarray()), axis=1)


credit_prepro.fit(np.array([0,2,3,4,5,6,7,8]).reshape(-1,1)) # PAY 5, 6
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,9].reshape(-1,1)).toarray()), axis=1)
credit_X = np.concatenate((credit_X, credit_prepro.transform(credit_X1[:,10].reshape(-1,1)).toarray()), axis=1)
...
mnist_MCC_knn = metrics.matthews_corrcoef(mnist_Y_test, mnist_Y_test_pred_knn)
mnist_prec_knn = metrics.precision_score(mnist_Y_test, mnist_Y_test_pred_knn, average='macro')
...
tb_knn.add_row(['Matthews correlation coeff', round(mnist_MCC_knn,4)])
tb_knn.add_row(['Precision', round(mnist_prec_knn,4)])
...
mnist_MCC_Dt = metrics.matthews_corrcoef(mnist_Y_test, mnist_Y_test_pred_Dt)
mnist_prec_Dt = metrics.precision_score(mnist_Y_test, mnist_Y_test_pred_Dt, average='macro')
...
tb_Dt.add_row(['Matthews correlation coeff', round(mnist_MCC_Dt,4)])
tb_Dt.add_row(['Precision', round(mnist_prec_Dt,4)])
...
mnist_MCC_SVM = metrics.matthews_corrcoef(mnist_Y_test, mnist_Y_test_pred_SVM)
mnist_prec_SVM = metrics.precision_score(mnist_Y_test, mnist_Y_test_pred_SVM, average='macro')
...
tb_SVM.add_row(['Matthews correlation coeff', round(mnist_MCC_SVM,4)])
tb_SVM.add_row(['Precision', round(mnist_prec_SVM,4)])
...
mnist_MCC_LoR = metrics.matthews_corrcoef(mnist_Y_test, mnist_Y_test_pred_LoR)
mnist_prec_LoR = metrics.precision_score(mnist_Y_test, mnist_Y_test_pred_LoR, average='macro')
...
tb_LoR.add_row(['Matthews correlation coeff', round(mnist_MCC_LoR,4)])
tb_LoR.add_row(['Precision', round(mnist_prec_LoR,4)])
...
#### no need to normalize categerial features ############
credit_X_train_mean[credit_noncat_len:] = 0
credit_X_train_std[credit_noncat_len:] = 1
...
credit_auc_knn = metrics.auc(credit_roc_knn[0],credit_roc_knn[1])
...
tb_knn.add_row(['Area Under ROC', round(credit_auc_knn,4)])
tb_knn.align['Parameter'] = 'l'
...
credit_auc_Dt = metrics.auc(credit_roc_Dt[0],credit_roc_Dt[1])
...
tb_Dt.add_row(['Area Under ROC', round(credit_auc_Dt,4)])
tb_Dt.align['Parameter'] = 'l'
...
#credit_X_train = pca.fit_transform(credit_X_train)

#credit_X_test = pca.transform(credit_X_test)
...
credit_auc_SVM = metrics.auc(credit_roc_SVM[0],credit_roc_SVM[1])
...
tb_SVM.add_row(['Area Under ROC', round(credit_auc_SVM,4)])
tb_SVM.align['Parameter'] = 'l'
...
credit_auc_LoR = metrics.auc(credit_roc_LoR[0],credit_roc_LoR[1])
...
tb_LoR.add_row(['Area Under ROC', round(credit_auc_LoR,4)])
tb_LoR.align['Parameter'] = 'l'
...
print('Total time taken',int(time.time()-glob_tim),'Sec')
"""