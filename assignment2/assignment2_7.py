...
print('Decision tree for MNIST data')
mnist_dot_data = tree.export_graphviz(Dt, out_file=None, filled=True, rounded=True, special_characters=True)

mnist_graph = graph_from_dot_data(mnist_dot_data)  
Image(mnist_graph.create_jpg())
...
############ Visualizing Decision tree  ##################################
print('Decision tree for Credit card data')
credit_dot_data = tree.export_graphviz(Dt, out_file=None, filled=True, rounded=True, special_characters=True)

credit_graph = graph_from_dot_data(credit_dot_data)  
Image(credit_graph.create_jpg())
...
print('Totoal time taken',int(time.time()-glob_tim),'Sec')
'''
1. for top 5 accuracy if only two classes got non zero probablites andother are zero
    oroginal class dont fall in this zero list what to do?
2. I shuffled the data only once then used four classifiers on that data
3. MNIST data was normalized by dividig train set with max(train) and tese set with max(train) again
4. credit card was normalized by sebtracing mean and divide it with std on both train and test set
